...
public class JobOfferNotificationFilter extends JobOfferNotificationFilter_Base {
    ...
    public static final Comparator<JobOfferNotificationFilter> COMPARATOR_BY_DEGREE = new Comparator<JobOfferNotificationFilter>() {
	...
	public int compare(JobOfferNotificationFilter o1, JobOfferNotificationFilter o2) {
	    ...
	    return String.CASE_INSENSITIVE_ORDER.compare(degree1.getName().getContent(Language.getUserLanguage()), degree2
		    .getName().getContent(Language.getUserLanguage()));
	}
	...
	subject = BundleUtil.getStringFromResourceBundle("JOB_BANK_RESOURCES",
		"message.jobbank.message.jobbank.email.notification.subject");
	body = BundleUtil.getFormattedStringFromResourceBundle("JOB_BANK_RESOURCES",
		"message.jobbank.message.jobbank.email.notification.body", jobOffer.getEnterpriseName().getContent(), jobOffer
			.getJobOfferType().getLocalizedName(), jobOffer.getFunctionDescription().getContent(), jobOffer
			.getRequirements().getContent(), jobOffer.getTerms().getContent(), jobOffer.getVacancies().toString(),
		jobOffer.getPresentationPeriod());
