...
public class ChangeContractEnterpriseActivity extends WorkflowActivity<EnterpriseProcess, EnterpriseContractInformation> {

    @Override
    public boolean isActive(EnterpriseProcess process, User user) {
	Enterprise enterprise = process.getEnterprise();
	return process.isProcessOwner(user) && !enterprise.isPendingAccountabilityType();
